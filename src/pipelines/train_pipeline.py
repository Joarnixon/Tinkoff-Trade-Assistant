...
class TrainPipeline:
    def __init__(self, model: Model, train_func: callable, validate_func: callable, data_manager: Union[DataManager, NullDataManager], cfg: DictConfig, model_cfg: DictConfig, train_cfg: DictConfig, logger, monitor=None):
        self.model = model
    ...
    def train(self, data: pl.DataFrame, figi: str):
        ...
        if self.model is None:
            if hasattr(self.model_cfg, 'hidden_layers'):
                ...
                self.model = instantiate(self.model_cfg, input_size=data.shape[1], hidden_layers=self.model_cfg.hidden_layers)
...
class TrainPipelineFactory:
    @staticmethod
    def create_ml_pipeline(cfg: DictConfig, model_cfg: DictConfig, train_cfg: DictConfig, model: Model, logger, data_manager: Optional[DataManager] = None) -> TrainPipeline:
    ...
    @staticmethod
    def create_nn_pipeline(cfg: DictConfig, model_cfg: DictConfig, train_cfg: DictConfig, model: Model, logger, data_manager: Optional[DataManager] = None) -> TrainPipeline:
        return TrainPipeline(
            model,
            train_nn,
            val_nn,
